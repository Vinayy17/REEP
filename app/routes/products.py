...
def get_products(
    ...
    total = query.count()
    products = (
        query.order_by(ProductModel.created_at.desc()).offset(offset).limit(limit).all()
    )
...
def create_product(
    ...
    existing = db.query(ProductModel).filter(ProductModel.sku == parent_sku).first()
        ...
        if existing.is_active == 0:
            ...
            if product_data.is_service == 1:
                ...
                for variant in product_data.variants or []:
                    ...
                    restored_variants.append(variant_dict)
    ...
    for variant in variants:
        variant["qr_code_url"] = generate_qr(
            {
                "type": "variant",
                "product_name": product_data.name,
                "sku": parent_sku,

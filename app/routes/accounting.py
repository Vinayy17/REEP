...
def cashbook(
    ...
    rows = base_query.order_by(LedgerEntry.entry_date.desc()).offset((page - 1) * limit).limit(limit).all()

    opening_balance = (
        db.query(func.coalesce(func.sum(LedgerEntry.credit - LedgerEntry.debit), 0))
        .filter(LedgerEntry.payment_mode == "cash", LedgerEntry.entry_date < rows[-1].entry_date if rows else None)
        .scalar()
        or 0
    )
...
def general_ledger(
    ...
    total = base_query.count()
    query = base_query.order_by(LedgerEntry.entry_date.desc(), LedgerEntry.created_at.desc())
    entries = query.offset((page - 1) * limit).limit(limit).all()

    opening_balance = (
        db.query(func.coalesce(func.sum(LedgerEntry.credit - LedgerEntry.debit), 0))
        .filter(LedgerEntry.entry_date < entries[-1].entry_date if entries else None)
        .scalar()
        or 0
    )

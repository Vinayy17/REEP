...
def finalize_draft(
    ...
    for item in items:
        ...
        if product.is_combo == 1:
            ...
            for combo in product.combo_items or []:

                combo_product = (
                    db.query(ProductModel)
                    .filter(ProductModel.id == combo["product_id"])
                    .with_for_update()
                    .first()
                )

                if not combo_product:
                    raise HTTPException(404, "Combo item product not found")

                required_qty = combo["qty"] * quantity

                if combo_product.stock < required_qty:
                    raise HTTPException(
                        400,
                        f"Insufficient stock for combo item {combo_product.name}"
                    )

                stock_before = combo_product.stock
                combo_product.stock = stock_before - required_qty

                flag_modified(combo_product, "stock")
...
def update_invoice_status(
    ...
    # 🚫 BLOCK if invoice already fully paid
    if invoice.balance_amount <= 0:
        raise HTTPException(
            status_code=400,
            detail="Invoice already fully paid. No more payments allowed."
        )

...
const Products = () => {
  ...
  const canManageProducts = ["admin", "store_handler"].includes(role)
...
const [stockMode, setStockMode] = useState("inward")
  ...
  const padSku = (num, size = 4) => {
    return String(num).padStart(size, "0")
  }
  const [variants, setVariants] = useState([
    { v_sku: "", variant_name: "", color: "", size: "", image_url: "" },
  ])
 ...
 const goToInventory = (sku, mode) => {
  setStockSku(sku)
  setStockMode(mode === "in" ? "inward" : "outward")
  ...
  useEffect(() => {
    const fetchRecentSkus = async () => {
      try {
        const res = await axios.get(`${API}/products/recent-skus?limit=2`)
        setRecentSkus(res.data.recent_skus || [])
      } catch {
  ...
  const lastSkuNumber = useMemo(() => {
    if (!recentSkus.length) return null
    return extractSkuNumber(recentSkus[0])
  }, [recentSkus])

  const nextSku = useMemo(() => {
    if (!recentSkus.length) return null

    const last = recentSkus[0]          // e.g. RR-0007
    const parts = last.split("-")
    if (parts.length < 2) return null

    const prefix = parts.slice(0, -1).join("-") // RR
    const num = extractSkuNumber(last)
    if (num == null) return null

    return `${prefix}-${padSku(num + 1)}`
  }, [recentSkus])
...
const lastCategoryQueryRef = useRef("")
const abortControllerRef = useRef(null)

const searchCategories = (text = "", force = false) => {
  ...
  if (!force) {
    // Same query again
    if (q === lastCategoryQueryRef.current) return

    // Less than 2 chars → ignore
  ...
  lastCategoryQueryRef.current = q
  ...
  }, 400) // ✅ professional debounce
}
// ✅ helper: updates recent SKUs instantly (no reload needed)
const updateRecentSkusAfterCreate = (newSku) => {
  if (!newSku) return

  setRecentSkus((prev) => {
    const updated = [newSku, ...prev.filter((s) => s !== newSku)]
    return updated.slice(0, 2) // keep only last 2
  })
}
...
const handleSubmit = async (e) => {
  ...
  // 🔐 ONLY admin can send cost_price
  if (role === "admin") {
    payload.cost_price = Number(formData.cost_price || 0)
  }
  ...
  try {
    if (editingProduct) {
      ...
      // ✅ FIX: update SKU suggestion immediately
      updateRecentSkusAfterCreate(formData.sku)
    }
...
setCategoryQuery(product.category_name || "")
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      // ⬅️ If last item on page deleted, go back one page
  ...
  const resetForm = () => {
    ...
    setVariants([{ v_sku: "", variant_name: "", color: "", size: "", image_url: "" }])
  }
      ...
      <div className="space-y-2">
        {canManageProducts && (
          <div className="hidden md:flex items-center justify-end gap-2">
            ...
            >
              <Button variant="outline" className="rounded-full px-4" onClick={() => setComboOpen(true)}>
                Combo
              </Button>
              <DialogTrigger asChild>
                <Button size={isMobile ? "sm" : "default"} className="rounded-full px-4 sm:px-6">
                  <Plus className="w-4 h-4 sm:mr-2" />
                  <span className="hidden sm:inline">{isMobile ? "Add" : "Add Product"}</span>
                </Button>
              </DialogTrigger>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          required
                          className="bg-muted/30 border-muted focus-visible:ring-primary h-12 font-mono"
                        ...
                        {lastSkuNumber !== null && (
                          <div className="flex items-center gap-3 text-[11px] font-mono">
                            ...
                            {nextSku && (
                              <button
                                type="button"
                                className="px-2 py-0.5 rounded-md bg-primary/10 text-primary font-bold hover:bg-primary/20 transition"
                                onClick={() =>
                                  setFormData((prev) => ({
                                    ...prev,
                                    sku: prev.sku || nextSku, // ❗ only fill if empty
                                  }))
                                }
                              >
                                Use {nextSku}
...
<Input
  ...
  onFocus={() => {
    if (!categoryResults.length) {
      searchCategories("", true) // load default
    }
  }}
  onChange={(e) => {
...
/>
                      ...
                      {/* ✅ SINGLE DROPDOWN */}
                      {categoryOpen && (
                        <div className="absolute z-50 mt-1 w-full bg-black border border-border rounded-lg max-h-56 overflow-auto">
                          {categoryLoading && (
                          ...
                          {categoryResults.map((cat) => (
                            <div
                              ...
                              onClick={() => {
                                const isSrv = cat.label.toLowerCase() === "services"
      ...
      <div className="space-y-2">
        ...
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-10 pr-12 h-11 rounded-2xl"
          />
          {isMobile && (
            <Button
              type="button"
              size="icon"
              variant="ghost"
              className="absolute right-1.5 top-1.5 h-8 w-8 rounded-xl"
      ...
      {isMobile ? (
        <div className="space-y-4">
          {products.map((p) => (
            <Card key={p.id} className="overflow-hidden border shadow-sm bg-muted/20">
                  ...
                  <Dialog>
                    <DialogTrigger asChild>
                      <img
                        ...
                        className="h-20 w-20 cursor-zoom-in object-cover rounded-2xl shadow-sm bg-background"
                        alt={p.name}
                      />
                    </DialogTrigger>
                    <DialogContent className="max-w-3xl rounded-3xl p-4">
                      <img
                        src={p.images?.[0] || "/placeholder.svg"}
        ...
        <Card className="border-muted shadow-lg overflow-hidden">
          <CardHeader className="bg-muted/30 border-b">
            <CardTitle className="flex items-center gap-2">
              ...
              Product Inventory (Page {page} of {totalPages})
            </CardTitle>
          </CardHeader>
  ...
  <DialogContent className="max-w-lg">
    ...
    <DialogHeader>
      <DialogTitle>
        {stockMode === "inward" ? "Stock Inward" : "Stock Outward"}
      </DialogTitle>
    </DialogHeader>
...
<Dialog open={comboOpen} onOpenChange={setComboOpen}>
  ...
  >
    <div className="flex flex-col h-full bg-background">
      ...
      {/* HEADER */}
      <div className="flex items-center justify-between px-4 py-3 sm:px-6 sm:py-4 border-b bg-muted/40 backdrop-blur shrink-0">
